...
try
    ...
    %Xml Hypnogram - need to figure out how to exclude this
    directory = char(Filenames(n,5));
    fname = char(Filenames(n,2));
    displaytext=['Get Hypnogram data: ' fname];
    disp(displaytext); set(handletext,'String',displaytext); drawnow;
    ...
    Nepochs = length(S.heartbeat-baseline-700001-nsrr.SleepStages.SleepStage);
    clear hypnogram
    hypnogram=NaN*zeros(Nepochs,1);
    for i=1:Nepochs
        hypnogram(i) = str2num(S.heartbeat-baseline-700001-nsrr.SleepStages.SleepStage{1,i}.Text);
    end %W=0,N1=1,N2=2,N3=3,R=5
    ...
    if ~isfield(S.heartbeat-baseline-700001-nsrr.ScoredEvents,'ScoredEvent')
        displaytext=['Warning: No scored events'];
        disp(displaytext); set(handletext,'String',displaytext); drawnow;
    end
    
    Nevents = length(S.heartbeat-baseline-700001-nsrr.ScoredEvents.ScoredEvent);
 
    clear Events
    for i=1:Nevents
        Events{i}.name = S.heartbeat-baseline-700001-nsrr.ScoredEvents.ScoredEvent{i}.Name.Text;
        Events{i}.start = StartTime + str2num(S.heartbeat-baseline-700001-nsrr.ScoredEvents.ScoredEvent{i}.Start.Text);
        Events{i}.duration = str2num(S.heartbeat-baseline-700001-nsrr.ScoredEvents.ScoredEvent{i}.Duration.Text);
    end %W=0,N1=1,N2=2,N3=3,R=5

function [x,Fs,Start_date,Start_time,Label,Dimension,Coef,Nmb_chans,N] = readedfrev3(fname,ch,t1,t2)
...
%
%   Revised by C. Parsley 15.6.2005 (add filter,patient and recording
%   information.
...
elseif length(fname)==0
	[fname,pname]=uigetfile('*.edf','Select European Data Format file');
	fname = [pname fname];
	fid = fopen(fname,'r');
else
	fid = fopen(fname,'r');
...
% First scan the header information
	...
	Start_date = char(fread(fid,8,'char')');
	Start_time = char(fread(fid,8,'char')');
	Hdr_size   = sscanf(char(fread(fid,8,'char')'),'%d');
	fseek(fid,44,0);
    dat_rec     = sscanf(char(fread(fid,8,'char')'),'%d');
	Blck_size  = sscanf(char(fread(fid,8,'char')'),'%d');
	Nmb_chans  = sscanf(char(fread(fid,4,'char')'),'%d');

	current = ftell(fid);
	fseek(fid,ch*16,0);
	Label      = char(fread(fid,16,'char')');
	fseek(fid,current+Nmb_chans*16+Nmb_chans*80,-1);

	current = ftell(fid);
	fseek(fid,ch*8,0);
	Dimension  = char(fread(fid,8,'char')');
	fseek(fid,current+Nmb_chans*8,-1);

	current = ftell(fid);
	fseek(fid,ch*8,0);
	Phys_min  = sscanf(char(fread(fid,8,'char')'),'%f');
	fseek(fid,current+Nmb_chans*8,-1);

	current = ftell(fid);
	fseek(fid,ch*8,0);
	Phys_max  = sscanf(char(fread(fid,8,'char')'),'%f');
	fseek(fid,current+Nmb_chans*8,-1);

	current = ftell(fid);
	fseek(fid,ch*8,0);
	Dig_min   = sscanf(char(fread(fid,8,'char')'),'%d');
	fseek(fid,current+Nmb_chans*8,-1);

	current = ftell(fid);
	fseek(fid,ch*8,0);
	Dig_max   = sscanf(char(fread(fid,8,'char')'),'%d');
	fseek(fid,current+Nmb_chans*8,-1);
    
    current = ftell(fid);
	fseek(fid,ch*80,0);
	Filt = char(fread(fid,80,'char')');
	fseek(fid,current+Nmb_chans*80,-1);
    
	Coef = [Phys_min Phys_max Dig_min Dig_max];
	
	Nmp_smps=zeros(1,Nmb_chans);
	for i=0:Nmb_chans-1
		Nmb_smps(i+1)  = sscanf(char(fread(fid,8,'char')'),'%d');
	end

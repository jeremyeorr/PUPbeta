function [SSres,Error,Rsq,Vchem,Varousal,Penalized_Error] = TheModelX1delay(Parameters,Data,i_1,polyfitorder)
...
Ttot_ser=Data(:,6);
    ...
    for k=i_1:length(VE)
        ...
        delayedVE(k)=VE(k-k_back)+(1-(delay_secs-sum(Ttot_ser(k-k_back+1:k-1)))/Ttot_ser(k-k_back))*(VE(k-k_back+1)-VE(k-k_back)); 
        delayedVE(1:i_1-1)=0;
    end
    %figure(1); plot(Data(:,4),[VE delayedVE])
if 0
LG0=Parameters(1);          %Estimated model parameters
tau1=Parameters(2);
else
%     LGn=Parameters(1); 
%     Tn=Parameters(2);   
% tau1=tan(pi-2*pi/Tn*delay)/(2*pi/Tn); 
% if tau1<0
%     tau1=0.1;
% end
    LG1=Parameters(1); 
...
%**************************************************************************
% Overall ventilatory drive is modelled as the combined effect of chemical
...
%**************************************************************************
    ...
    if 1 %Subtract the polynomial drift, but leaving the 'mean' error untouched. Solutions whose mean is non-zero will not be favored.

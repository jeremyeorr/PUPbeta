function [Error,Vchem,Varousal,LoopGainParameters,BestParameters,BestSSres,FitQuality,i_1,i_end,lowerSEM,upperSEM,CI_parameters] = FindBestModelParametersCI(Data,VraOn,Veupnea,polyfitorder) 
...
%     [LG1          tau         Err1[0]          VRA       Delay    ]
%**************************************************************************
start=[ 0.7         10          0                0         NaN   ]; % A reasonable estimate of a starting physiological value for each parameter
lower=[ 0.05         2           -Veupnea*3      0          1     ]; % Minimum conveivable physiological value for each parameter
upper=[ 3           180         Veupnea*3       Veupnea*3  5     ]; % Maximum conveivable physiological value for each parameter
%**************************************************************************
%**************************************************************************
...
for dd=1:length(delay_breaths)
    ...
    %lower(2)=2*delay_secs;
    %upper(2)=3.9*delay_secs;
    
    OPTIONS = optimset('Display','off','TolX',1e-4,'TolFun',1e-4,'MaxIter',400,'MaxFunEvals',150,'Algorithm','interior-point');
    Parameters=start;
    [final_parameters_1,SSres_1,~,~] = fmincon(@(Parameters) TheModelX1(Parameters,Data,i_1,delayed_VE{dd},polyfitorder,delay_secs),Parameters,[],[],[],[],lower,upper,@(Parameters) model_nonlinear_constraints(),OPTIONS); %@(Parameters) model_nonlinear_constraints() is now [],[]
    
    Parameters=upper;
    [final_parameters_2,SSres_2,~,~] = fmincon(@(Parameters) TheModelX1(Parameters,Data,i_1,delayed_VE{dd},polyfitorder,delay_secs),Parameters,[],[],[],[],lower,upper,@(Parameters) model_nonlinear_constraints(),OPTIONS);

    Parameters=lower;
    [final_parameters_3,SSres_3,~,~] = fmincon(@(Parameters) TheModelX1(Parameters,Data,i_1,delayed_VE{dd},polyfitorder,delay_secs),Parameters,[],[],[],[],lower,upper,@(Parameters) model_nonlinear_constraints(),OPTIONS);
    
    Parameters=(start-lower)/2;
    [final_parameters_4,SSres_4,~,~] = fmincon(@(Parameters) TheModelX1(Parameters,Data,i_1,delayed_VE{dd},polyfitorder,delay_secs),Parameters,[],[],[],[],lower,upper,@(Parameters) model_nonlinear_constraints(),OPTIONS);
    
    Parameters=(upper-lower)/2;
    [final_parameters_5,SSres_5,~,~] = fmincon(@(Parameters) TheModelX1(Parameters,Data,i_1,delayed_VE{dd},polyfitorder,delay_secs),Parameters,[],[],[],[],lower,upper,@(Parameters) model_nonlinear_constraints(),OPTIONS);
    %>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    [BestParametersForEachDelay(:,dd),Optimal_SSres(dd),~,~] = fmincon(@(Parameters) TheModelX1(Parameters,Data,i_1,delayed_VE{dd},polyfitorder,delay_secs),Parameters,[],[],[],[],lower,upper,@(Parameters) model_nonlinear_constraints(),OPTIONS);
...
[BestSSres,Error,Rsq,Vchem,Varousal] = TheModelX1(BestParameters,Data,i_1,delayed_VE{ind},polyfitorder,delay_secs);
...
if ~noJacobian
    ...
    for i=1:length(BestParametersDelay)
        ...
        [FF,Error_,~,VchemA,~,Penalized_Error] = TheModelX1(BestParameters,Data,i_1,delayed_VE{ind},polyfitorder,delay_secs);
        modelY_=VchemA;
